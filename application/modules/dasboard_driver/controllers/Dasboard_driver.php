 ...
 * Time: 10:04 AM
 */
class Dasboard_driver extends MX_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model('dasboard_driver/driver');
        $this->load->model('dasboard_driver/Captcha');
    ...
    {
        if ($this->session->logged_in_driver)
        {
            $data['datas'] = "";
            $this->template->set_template('backoffice/template_driver');
            $this->template->content->view('dasboard_driver/dashboard', $data);
        ...
        } else
        {
            ...
            $this->load->view('dasboard_driver/login-page', $data);
        }
    }
    ...
    public function verifyLogin()
    {
        ...
        $this->form_validation->set_rules('password', 'Password', 'trim|required|callback_verify_driver');
        ...
        } else
        {
            ...
            $this->session->set_userdata('logged_in_driver', $this->driver->login($username, $password));
    ...
    public function verify_driver()
    {
        ...
        $password = $this->input->post('password');
        //query the database
        $result = $this->driver->login($username, $password);
        if ($result)
        {
            return true;
        } else
        {
            $this->form_validation->set_message('verify_driver', 'Invalid username or password');
            return false;
        }
    }
