...
class Manage_menumanager extends MX_Controller {
     ...
     public function index()
    {
        ...
        {
            $data['datas'] = $this->Data_menumanager
     ...
     public function search()
    {
        ...
        $this->initialize_pagination(base_url("manage_menumanager/search"),
            'menumanager',
            'Data_menumanager',
            $this->Data_menumanager->countSearchData(
                'menumanager',
                null,
                null,
    ...
    public function tambahMenuManager()
    {
        if ($this->isPost())
        {
            ...
            $order = $this->Data_menumanager->getTopOrderPlusOne();
            if ($this->input->post('top_menu') != 0)
            {
                $order = $this->Data_menumanager->getParentOrder($this->input->post('top_menu'));
                $this->Data_menumanager->incrementOrder($order);
            }
            $this->Data_menumanager->addData('menumanager', array(
        ...
        {
            $data['top_menus'] = $this->Data_menumanager->getAllData('menumanager');
            $this->template->set_template('backoffice/template');
    ...
    public function editMenuManager($menuid)
    {
        if ($this->isPost())
        {
            ...
            if ($this->input->post('top_menu') != 0)
            {
                $order = $this->Data_menumanager->getParentOrder($this->input->post('top_menu'));
                $this->Data_menumanager->incrementOrder($order);
                $this->Data_menumanager->updateData(array(
                    'MENU_PARENT'     => $this->input->post('top_menu'),
                    'MENU_TITLE'      => $this->input->post('title_id'),
                    'URL'             => $this->input->post('url'),
                    'MENU_SORT'       => 1,
        ...
        {
            $data['top_menus'] = $this->Data_menumanager->getAllData('menumanager');
            $data['menu'] = $this->Data_menumanager->getSpecificData('menumanager', array('id' => $menuid));
            $this->template->set_template('backoffice/template');
    ...
    {
        $this->Data_menumanager->deleteData('menumanager', array('id' => $id));
        $this->session->set_flashdata('msg', 
                '<div class="alert alert-success">
                    <h4>Success </h4>
    ...
    {
        $this->template->set_template('backoffice/template');
        $this->template->content->view('manage_menumanager/sorting_menumanager', $data);
        $this->template->publish();
    }
    ...
    public function showmenus()
    {
        $categories = array();
        $pool = array();
        $q = $this->Data_menumanager->returnparentmenus();
        foreach ($q as $row)
        {
    ...
    {
        $this->Data_menumanager->resetMenuManagerOrder();
        ...
        if (is_array($response))
        {
            ...
            {
                $menuid = $this->Data_menumanager->updateMenu($block['id'], 0, $topmenusorder);
    ...
    public function updateChildSubmenus($menuid, $e)
    {
        ...
        foreach ($e as $key => $block)
        {
            ...
            $menu = $this->Data_menumanager->updateMenu($block['id'], $menuid, $topmenusorder);
            if (isset($block['children']))
            {
