...
class Forgotpassword extends MX_Controller {
    ...
    public function index()
    {
        ...
        $crud->set_table('forgot_password');
        $crud->set_theme('bootstrap');
        $crud->set_language("english");

        $crud->set_subject('Forgot Password');


        //kolom tabelnya apa aja yg di tampilin di tabel data
        $crud->columns('email','last_deposit_amount'); 
        //kolom apa aja yang mau di tampilin saat tambah data
        $crud->add_fields('email','last_deposit_amount');
        //kolom apa aja yang mau di tampilin saat edit data
        $crud->edit_fields('email','last_deposit_amount');
        //kolom apa aja yang mau di tampilin saat view detail data
        $crud->set_read_fields('email','last_deposit_amount');

        //set kolom yang mandatory(wajib diisi)
        $crud->required_fields('email','last_deposit_amount');
        
        //data diorder berdasarkan apa
        $crud->order_by('id_forgot_password','desc');
        ...
        try {
            $crud->callback_after_insert(array($this, 'call_back_insert'));
            ...
            $this->template->content->view('forgotpassword/index', $output);
    ...
    function call_back_insert($post_array,$primary_key)
    {
        ...
        $this->db->update('withdraw',$data_update,array('id_forgotpassword' => $primary_key));
        return true;
