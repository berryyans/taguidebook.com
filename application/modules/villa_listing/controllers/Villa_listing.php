<?php
class Villa_listing extends MX_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('villa_listing/Villa_listing_model');
    }
    public function index()
    {
        ...
        $config["total_rows"] = $this->Villa_listing_model->record_count($data['search_string']);
        ...
        $config["base_url"] = base_url().'villa_listing/index/'.$data['sort_by'].'/'.$orderBy.'/'.$data['search_string'];
        ...
        $data["data"] = $this->Villa_listing_model->get_villas($config["per_page"], $offset, $data['sort_by'], $data['sort_order'], $data['search_string']);
        ...
        $this->template->content->view('villa_listing/index', $data);
    ...
    {
        //only this section
        $data['result']    = $this->Villa_listing_model->getSpecificData('villa', array('id_villa' => $id));
        //only this section
        ...
        $this->template->keyword = 'Godrive';
        $this->template->header->view('frontend/partials/header');//, $data);
        $this->template->content->view('villa_listing/detail', $data);
        $this->template->footer->view('frontend/partials/footer');//, $data);
        $this->template->publish();
    }
