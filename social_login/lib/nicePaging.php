...
class nicePaging{
	...
	public function createPaging($link){
		...
		$paging='<div class="row" align="center">
      				<ul class="pagination">';
		if($this->page>1){
			$paging.='<li><a href="'.$link.$this->separator.'pages=1" title="First page">&lt;&lt;</a></li>';
			$paging.='<li><a href="'.$link.$this->separator.'pages='.($this->page-1).'" title="Previous page">&lt;</a></li>';
		}
		
		if($start>$this->maxPages){
			$paging.='<li><a href="'.$link.$this->separator.'pages='.($start-1).'" title="Page '.($start-1).'">...</a></li>';
		}
		
		for($i=$start;$i<=$end;$i++){
			if($this->page==$i){
				$paging.='<li class="active"><a href="#" title="Page '.$i.'">'.$i.'</a></li>';
			}
			else{
				$paging.='<li><a href="'.$link.$this->separator.'pages='.$i.'" title="Page '.$i.'">'.$i.'</a></li>';
			}
		}
		
		if($end<$this->totalPages){
			$paging.='<li><a href="'.$link.$this->separator.'pages='.($end+1).'" title="Page '.($end+1).'">...</a></li>';
		}
		
		if($this->page<$this->totalPages){
			$paging.='<li><a href="'.$link.$this->separator.'pages='.($this->page+1).'" title="Next page">&gt;</a></li>';
			$paging.='<li><a href="'.$link.$this->separator.'pages='.$this->totalPages.'" title="Last page">&gt;&gt;</a></li>';
		}
